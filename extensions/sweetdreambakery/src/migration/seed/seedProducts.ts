...
import { info, success, error, warning } from "@evershop/evershop/lib/log";
...
export async function seedProducts(
  ...
  // Get flavor and size attribute IDs
  const flavorAttribute = await select()
    .from("attribute")
    .where("attribute_code", "=", "flavor")
    .load(pool);
  ...
  if (!flavorAttribute || !sizeAttribute) {
    error(
      "Flavor and Size attributes must be seeded first. Run: npm run seed -- --attributes"
    );
    return;
  }
